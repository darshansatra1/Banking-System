...
const UserDepositHistory = ({ role }) => { // Accept 'role' as a prop
    const { userId } = useParams();
    console.log("user id in deposit history is " + userId);
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            axios.get(`http://localhost:8080/admin/user/${userId}/deposit`,
            {
                headers: {
